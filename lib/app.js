...
function SetEventHandlers() {
    ...
    $(".stickyContainer").on("drag", function (event, ui) {
        //console.log(Math.floor(ui.position.left / (screenWidth / 5)));
        $(this).attr("data-col", Math.floor(ui.position.left / (screenWidth / 5)));
    });
}
...
function createSticky() {
    ...
    var initLeft = eval(t1x - 100) < 10 ? 10 : eval(t1x - 100)


    screenWidth ? screenWidth - initWidth : eval(t1x - 100);
    ...
    var newDiv = $(".stickyTemplate").clone().removeClass("stickyTemplate").addClass("stickyContainer")
        ...
        .attr("data-col", Math.floor(t1x / (screenWidth / 5)));
    ...
    $(newDiv).children(".stickyClose").on("touchend", function (eventArgs) {
        $(this).parent().remove();
        eventArgs.preventDefault();
        eventArgs.stopPropagation();
    })
