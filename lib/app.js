...
//  Initially created by Anderw Cote
//  Track the changes on BitBucket: https://maroonedsia@bitbucket.org/kaoninteractive/scrumboard.git
...
function SetEventHandlers() {
    ...
    $(".stickyContainer").on("drag", function (event, ui) {
        newCol = Math.floor(ui.position.left / (screenWidth / 5));
        if (newCol !== currentCol && newCol >= 0 && newCol <= 4) {
            changeColorAnim($(this), currentCol, newCol);
...
function createSticky() {
    ...
    $(".frame").append(newDiv);
    // this focus is not working anymore!
    $(newDiv).children(".stickyBody").children(".stickyTaskOwner").focus();
...
function changeColorAnim(stickyContainer, currentCol, newCol) {
    ...
    // remove anim class after it's done.
